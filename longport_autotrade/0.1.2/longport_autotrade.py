...
TRAILING_STOP_PCT = 0.05        # 移动止盈回撤阈值 (从最高点回撤 5% 卖出)
BUY_MOMENTUM_THRESHOLD = 0.01   # 买入动量阈值 (日内涨幅超过 1% 且趋势向上才买)
...
def dxyz_strategy_logic(symbol, config, running_event):
    """
    DXYZ 专用策略: 结合趋势跟踪与移动止损
    目标: 
    1. 不亏损 -> 严格的 Stop Loss
    2. 收益高 -> Trailing Stop (价格上涨时提高止损线)
    """
        ...
        try:
            ...
            if position:
                ...
                if current_price > highest_price:
                    ...
                    logger.info(f"{symbol} 创新高: {highest_price}, 移动止损线上移")
                ...
                # 策略 2: 移动止盈 (保住利润)
                # 如果有利润 (例如 > 2%) 且 回撤超过阈值，则止盈
                elif pnl_pct > 0.02 and drawdown_pct > TRAILING_STOP_PCT:
                    trader.execute_sell(symbol, quantity, current_price, reason=f"触发移动止盈 (回撤 {drawdown_pct:.2%})")

                else:
                    logger.info(f"持仓中 {symbol}: 现价 {current_price} | 成本 {entry_price} | 浮盈 {pnl_pct:.2%}")

            # --- 场景 B: 空仓 (监控买入) ---
            else:
                # 策略: 简单的日内动量策略
                # 如果当前价格比开盘价高 X%，且比前一次查询价格高 (简单的趋势判断需结合更多指标，这里简化)
                
                day_change_pct = (current_price - open_price) / open_price
                
                # 只有当日常涨幅超过阈值，才考虑入场 (追涨策略)
                if day_change_pct > BUY_MOMENTUM_THRESHOLD:
                    ...
                    logger.info(f"观察中 {symbol}: 现价 {current_price} | 日涨幅 {day_change_pct:.2%} (未达阈值)")
...
def main():
    # 打印欢迎信息
    print("========================================")
    print("   Longport AutoTrade v1.0 (DXYZ Special)")
    print("========================================")
