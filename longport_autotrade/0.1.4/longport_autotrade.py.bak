...
def dxyz_strategy_logic(symbol, config, running_event):
    ...
    while running_event.is_set():
        ...
        try:
            ...
            # [关键修复 1] 剔除后缀，适配 Finnhub API (Finnhub 不认识带 .US 的代码)
            finnhub_symbol = symbol.split('.')[0] 
            
            while retry_count < 3:
                try:
                    quote = finnhub_client.quote(finnhub_symbol) # 使用剔除后缀后的代码
                    break # 成功获取则跳出循环
            ...
            # [修改] 使用 .get() 安全获取，防止报 KeyError
            current_price = float(quote.get('c', 0))  # 当前价格
            open_price = float(quote.get('o', 0))     # 今日开盘价
            prev_close = float(quote.get('pc', 0))    # 昨收价 (Previous Close)
            # [新增] 3.a 读取成交量
            current_volume = float(quote.get('v', 0)) # v 是当日常量

            # [关键修复 2] 如果价格为0，大声报错而不是静默跳过，防止假死
...
def main():
    # 打印欢迎信息
    print("========================================")
    print("   Longport AutoTrade v0.1.4 (Bug Fixed)")
    print("========================================")
